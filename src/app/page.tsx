...
import {
  ...
  BarChart2, Globe, Clock, CheckCircle, XCircle,
  Database, Moon, Sun, LogOut, User,
} from 'lucide-react'
...
export default function HomePage() {
  ...
  const handleLogout = async () => {
    try { await authApi.logout() } catch { /* token already invalid is fine */ }
    clearAuth()
        ...
        <header className="bg-[var(--panel-bg)] border-b border-[var(--border)] sticky top-0 z-30">
          <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              {/* ── User chip + Logout ── ADDED ────────────────────────────── */}
              <div className="flex items-center gap-1 pl-2 border-l border-[var(--border)]">
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[var(--surface-2)] text-[var(--muted)] text-sm">
                  <User size={14} />
                    ...
                    <tr key={form.id} className="hover:bg-[var(--surface-2)] transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-1 justify-end">
                          <Link href={`/forms/preview/${form.id}`} className="p-1.5 rounded-md hover:bg-[var(--surface-2)] text-[var(--muted)] transition-colors" title="Preview">
                            <Eye size={15} />
                          </Link>
                          <Link href={`/builder/${form.id}`} className="p-1.5 rounded-md hover:bg-[#eef2ff] dark:hover:bg-[#1e1b4b] text-[var(--brand)] transition-colors" title="Edit">
                            <Edit2 size={15} />
                          </Link>
                          <Link href={`/submissions/${form.id}`} className="p-1.5 rounded-md hover:bg-[#f0fdf4] dark:hover:bg-emerald-900/20 text-[#10b981] transition-colors" title="Submissions">
                            <BarChart2 size={15} />
                          </Link>
                          <button onClick={() => handleDelete(form.id)} className="p-1.5 rounded-md hover:bg-red-50 dark:hover:bg-red-900/20 text-[#ef4444] transition-colors" title="Delete">
                            <Trash2 size={15} />
                          </button>
                        </div>
                      </td>
